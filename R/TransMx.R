...
TransMx <- function(strings.vec, indiv = FALSE){
   ...
   # 0.0 make sure strings.vec is characters
   strings.vec <- as.character(strings.vec)

   # 0.1 split string into single characters
   str.sp.list <- lapply(strings.vec, function(x) strsplit(x, ''))

   # 0.2 vector of unique characters from above, sorted
   str.spu <- sort(unique(unlist(str.sp.list)))

   # 0.3 number of unique characters
   num.str.spu <- length(str.spu)

   # 0.4 number sequence from 1 to num.str.spu
   ...
   trans.df.list <- lapply(strings.vec, TransLoca)
   ...
   # 4.1 grand total number of all transitions in all strings
   trans.total <- sum(trans.df.sum.df)  

   # 4.2 normalized transition numbers in all strings
   trans.df.sum_norm.df <- trans.df.sum.df / trans.total

   # 4.3 round to 4 decimals if not 0
   trans.df.sum_norm.df <- apply(trans.df.sum_norm.df, 1:2, function(x){
                                    ifelse (x > 0, round(x, digits = 4), "0")
                              })
 
   # 4.4 first characters (i.e., 'From') of transitions    
   ...
   if(indiv == TRUE){ 
      ...
      # 7.2 get the number of string 
      num_strings <- length(strings.vec)
              
      # 7.3 prepare out files names   
      num_strings.c <- sprintf("%02d", 1:num_strings)

       # 7.4 out file names (based on the input vec), one for each string
      out.file.names <- paste0(strings.vec_name, "_", num_strings.c, "mx", ".txt")

      # 7.5 Write output transition matrix file for each string
